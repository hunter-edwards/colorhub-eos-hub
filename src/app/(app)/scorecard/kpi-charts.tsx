...
const KPI_DEFINITIONS: Record<string, {
  ...
  'Runs Completed': {
    description: 'Count of individual runs shipped this week.',
    fields: 'field_972 (shippedButtonDateStamp)',
    formula: 'A run is "completed" when field_972 (shipped-button click date) falls in the week. Result = count of those runs.',
    color: '#2563eb',
    chartType: 'bar',
    drilldown: true,
  },
  'Jobs Completed': {
    description: 'Count of parent jobs fully shipped this week.',
    fields: 'field_972 (shippedButtonDateStamp), field_534 (parent job #), field_1589 (customer #), field_535 (part #)',
    formula: 'Group runs by customer + parent job. A job counts as completed when EVERY run (across every part) has field_972 set. Week = MAX(field_972) across the job\'s runs.',
    color: '#16a34a',
    chartType: 'bar',
    drilldown: true,
  },
  ...
  'Avg Days Shipped\u2192Invoiced': {
    ...
    fields: 'object_10.field_121 (postedDate), object_1.field_972 (shippedButtonDateStamp)',
  ...
  'Avg Days Order\u2192Complete': {
    ...
    fields: 'field_969 (order received date), field_972 (shippedButtonDateStamp)',
    formula: 'For each run shipped this week: days = field_972 \u2212 field_969. Average across runs that have both dates. Lower is better.',
    color: '#d97706',
    chartType: 'line',
    drilldown: true,
  },
  'On-Time Delivery %': {
    description: 'Percentage of runs shipped on or before the customer due date.',
    fields: 'field_497 (orderDueDate, equation), field_972 (shippedButtonDateStamp)',
    formula: 'For runs with a due date: on-time if field_972 <= field_497. Result = (onTimeCount / totalWithDueDate) * 100.',
    color: '#9333ea',
    chartType: 'line',
    drilldown: true,
  },
